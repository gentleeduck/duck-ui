...
Use `MotionPagination` with `MotionPaginationLink`, `MotionPaginationPrevious`, and `MotionPaginationNext` for staggered entrance animations and tap feedback powered by [motion](https://motion.dev). Each link uses a `scaleIn` entrance with a bouncy spring transition, staggered by 50ms via the `index` prop.
