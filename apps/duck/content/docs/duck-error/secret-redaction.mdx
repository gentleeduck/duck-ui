...
- **`cause`** -- set by `asError`/`rethrowError` to the original wrapped value, and is not part of `toJSON()`'s output at all. If you log `err.cause` directly (for internal diagnostics, not a client response), it is *not* redacted -- that is the whole reason it exists as a separate channel from the client-facing body.
