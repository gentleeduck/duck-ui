...
description: Persist policies, roles, assignments, and subject attributes through Prisma Client, with the four reference models and their real behaviour.
...
<PackageManagerTabs bun="bun add @gentleduck/iam @prisma/client" npm="npm i @gentleduck/iam @prisma/client" pnpm="pnpm add @gentleduck/iam @prisma/client" yarn="yarn add @gentleduck/iam @prisma/client" />
...
<Callout tone="info" title="No `onPolicyError` hook">
...
<MermaidDiagram chart={`erDiagram
    AccessRole ||--o{ AccessAssignment : "roleId onDelete Cascade"
    AccessPolicy {
        String id PK
        String name
        String description "nullable"
        Int version "default 1"
        String algorithm "default deny-overrides"
        Json rules
        Json targets "nullable"
        DateTime createdAt
        DateTime updatedAt
    }
    AccessRole {
        ...
        String description "nullable"
        Json permissions
        StringList inherits "default empty"
        String scope "nullable, NULL means global"
        Json metadata "nullable"
        DateTime createdAt
        DateTime updatedAt
    }
    AccessAssignment {
        String id PK "cuid default"
        String subjectId FK "indexed"
        String roleId FK "cascades"
        String scope "nullable, NULL means global"
...
<Callout tone="warning" title="The read is a fast path, not the guarantee">
...
<Callout tone="warning" title="Temporal grants and per-grant attributes are refused">
...
<MermaidDiagram chart={`flowchart TD
    A["updateAssignmentScope(subject, role, from, to)"] --> B["findMany where scope = from, take 1"]
    B --> C{"source row exists?"}
    C -- "no" --> D["return false, engine falls back to revoke + assign"]
    C -- "yes" --> J{"from and to equal? compared in JavaScript"}
    J -- "yes" --> H
    J -- "no" --> E["deleteMany where scope = to"]
    E --> F["updateMany where scope = from, set scope = to"]
    F --> G{"count > 0?"}
    G -- "yes" --> H["return true"]
    G -- "no" --> I["return false"]`} />

<Callout tone="warning" title="CAVEAT: the source-row check must come first">
...
<Callout tone="warning" title="The `NOT` clause is gone, and that was the fix">
