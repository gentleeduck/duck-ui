...
| `options` | `{ shiftKey?: boolean }` | Optional. In `multi-range` mode only: if `shiftKey` is true and the clicked date falls within an existing completed range longer than 5 days, splits that range to exclude the clicked date (shrinks from an edge, or splits in two if clicked in the middle); ignored otherwise |
