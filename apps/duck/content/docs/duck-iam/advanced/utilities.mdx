...
description: Every exported matcher, resolver, condition primitive, cache, and key helper - signatures, exact semantics, and runnable examples
section: Advanced
---
...
<Callout tone="warning" title="The condition primitives are renamed on the way out">
...
<MermaidDiagram chart={`flowchart TD
    R["A rule and a request"] --> A["matchesAction on every rule.actions entry"]
    R --> B{"Does the pattern or the resource type contain a dot?"}
    B -- "yes" --> C["matchesResourceHierarchical"]
    B -- "no" --> D["matchesResource"]
    R --> E["iamEvalConditionGroup on rule.conditions"]
    E --> F["iamEvalCondition per leaf"]
    F --> G["resolve(request, cond.field) -> actual"]
    F --> H["iamResolveValue(request, cond.value) -> expected"]
    G --> I["iamEvaluateOperator(op, actual, expected)"]
    H --> I
    A --> J["matched = action AND resource AND conditions"]
    C --> J
    D --> J
    I --> J
    K["matchesScope"] --> L["RBAC permission scoping, separate from rule matching"]`} />
...
<Callout tone="warning" title="A bare pattern is not a parent">
...
<Callout tone="warning" title="These refuse rather than answering false">
...
<Callout tone="warning" title="Empty string is a real segment">
...
<Callout tone="warning" title="Refuse a policy row, drop a role row">
