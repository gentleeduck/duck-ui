---
title: Benchmarks
description: Measured performance for @gentleduck/iam: the methodology, the hardware, and where a warm check actually spends its time
section: Benchmarks
---
...
<Callout tone="warning" title="These numbers have a shelf life">
...
<Callout tone="info" title="Fixtures differ between suites">
...
<Callout tone="info" title="mode defaults to production since 5.9.0">
...
<Callout tone="warning" title="Measured at 2.2.0, not re-measured for 5.9.0">
...
<Callout tone="warning" title="bun run benchmark does not currently run">
