...
description: deny-overrides, allow-overrides, first-match, and highest-priority - a decision diagram and a worked example for each, plus the defaultEffect fallback
section: Core
---
...
<MermaidDiagram chart={`flowchart TD
    R["policy.rules in source order"] --> F["keep rules whose actions and resources match"]
    F --> C["keep rules whose condition tree evaluates true"]
    C --> M["matched: rule and effect pairs, source order preserved"]
    M --> ALG{"policy.algorithm"}
    ALG -- "deny-overrides" --> D1["first deny, else first allow"]
    ALG -- "allow-overrides" --> D2["first allow, else first deny"]
    ALG -- "first-match" --> D3["highest priority, ties by source order"]
    ALG -- "highest-priority" --> D4["highest priority, ties by source order"]
    D1 --> OUT["IDecision: effect, rule, reason"]
    D2 --> OUT
    D3 --> OUT
    D4 --> OUT
    M -- "matched is empty" --> DEF["effect = defaultEffect, no rule, reason: No matching rules"]
    DEF --> OUT`} />
...
<MermaidDiagram chart={`flowchart TD
    S["matched rules"] --> Q1{"any rule with effect deny?"}
    Q1 -- "yes" --> DENY["deny - reason: Denied by rule id (first deny in source order)"]
    Q1 -- "no" --> Q2{"any rule with effect allow?"}
    Q2 -- "yes" --> ALLOW["allow - reason: Allowed by rule id (first allow in source order)"]
    Q2 -- "no" --> DEF["defaultEffect - reason: No matching rules. Defaulted to deny"]`} />
...
<MermaidDiagram chart={`flowchart TD
    S["matched rules"] --> Q1{"any rule with effect allow?"}
    Q1 -- "yes" --> ALLOW["allow - reason: Allowed by rule id (first allow in source order)"]
    Q1 -- "no" --> Q2{"any rule with effect deny?"}
    Q2 -- "yes" --> DENY["deny - reason: Denied by rule id (first deny in source order)"]
    Q2 -- "no" --> DEF["defaultEffect - reason: No matching rules. Defaulted to deny"]`} />
...
<Callout tone="warning" title="allow-overrides makes a policy unable to veto">
...
<MermaidDiagram chart={`flowchart TD
    S["matched rules"] --> E{"matched is empty?"}
    E -- "yes" --> DEF["defaultEffect - reason: No matching rules. Defaulted to deny"]
    E -- "no" --> INIT["best = matched[0]"]
    INIT --> SCAN["scan matched[1..n] in source order"]
    SCAN --> CMP{"priority(current) > priority(best)?"}
    CMP -- "yes" --> SET["best = current"]
    CMP -- "no (lower or equal)" --> KEEP["keep best - equal priority never displaces"]
    SET --> SCAN
    KEEP --> SCAN
    SCAN --> OUT["best.effect - reason: First match: rule id (effect)"]`} />
...
<Callout tone="info" title="Source order holds in production mode too">
...
<MermaidDiagram chart={`flowchart TD
    S["matched rules"] --> E{"matched is empty?"}
    E -- "yes" --> DEF["defaultEffect"]
    E -- "no" --> RED["reduce: keep current only when priority(current) > priority(best)"]
    RED --> P{"priority is a finite number?"}
    P -- "yes" --> USE["rank by that number"]
    P -- "no (NaN or Infinity)" --> ZERO["rank as 0 - the row still competes"]
    USE --> OUT["best.effect - reason: Highest priority: rule id (p=value)"]
    ZERO --> OUT`} />
...
<Step>Add a trailing catch-all allow</Step>
...
<Step>Narrow the policy with targets</Step>
...
<Step>Flip the engine default</Step>
