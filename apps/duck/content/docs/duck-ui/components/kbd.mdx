...
Use `MotionKbd` for a staggered `scaleIn` entrance with a bouncy spring transition on keyboard shortcuts, powered by [motion](https://motion.dev).
...
Same props as `Kbd` plus an optional `index` prop for stagger delay (30ms per index). Adds a `scaleIn` entrance with a bouncy spring transition. Requires the `motion` package.
