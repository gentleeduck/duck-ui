...
description: Every method on the object returned by createIam - verified signature, what it builds, what it validates, and a runnable example
section: Advanced
---
...
<MermaidDiagram chart={`classDiagram
    ...
    class When
    class IamEngine
    class IamValidateResult
    IAccessConfig --> RoleBuilder : defineRole
    IAccessConfig --> PolicyBuilder : definePolicy
    IAccessConfig --> RuleBuilder : defineRule
    IAccessConfig --> When : when
    IAccessConfig --> IamEngine : createEngine
    IAccessConfig --> IamValidateResult : validateRoles and validatePolicy
    PolicyBuilder --> RuleBuilder : rule
    RuleBuilder --> When : when and whenAny
    RoleBuilder --> When : grantWhen`} />
...
<Callout tone="warning" title="Cycles do not fail validation">
