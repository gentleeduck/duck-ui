---
title: Primitives
description: Every core duck-iam type with its exact exported name and field list - subject, resource, environment, request, role, policy, rule, condition, and decision
section: Core
---
...
<MermaidDiagram chart={`classDiagram
    ...
    IAccessRequest --> ISubject
    IAccessRequest --> IResource
    IAccessRequest --> IEnvironment
    ISubject --> IScopedRole
    IPolicy --> IRule
    IRule --> IConditionGroup
    IConditionGroup --> ICondition
    IRole --> IPermission
    IPermission --> IConditionGroup
    IDecision --> IRule`} />
...
<MermaidDiagram chart={`flowchart LR
    AV["IamPrimitives.Attributes"] --> S["subject.attributes"]
    AV --> R["resource.attributes"]
    AV --> E["environment (custom keys)"]
    AV --> SR["subject.scopedRoles[].attributes"]
    AV --> M["rule.metadata / role.metadata"]
    S --> C["Readable by conditions"]
    R --> C
    E --> C
    SR --> C
    M --> N["Not readable by conditions"]`} />
...
<Callout tone="warning" title="Attribute bags are bounded on the write path">
...
<Callout tone="warning" title="roles must be an array">
...
<Callout tone="info" title="now is injected, timestamp is not">
...
<Callout tone="warning" title="Never read allowed on a per-policy NotApplicable decision">
