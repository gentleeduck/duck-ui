...
description: Define a role, seed a memory adapter, create an engine, and answer your first authorization question in about twenty lines
section: Course
order: 1
---
...
<MermaidDiagram chart={`flowchart LR
    A["alice"] --> E["IamEngine"]
    R["read on document"] --> E
    E --> AD["IamMemoryAdapter"]
    AD --> |"roles + assignments"| E
    E --> Y["allowed: true"]
    E --> N["delete on document<br/>allowed: false"]`} />
...
<Callout tone="warning" title="Both methods fail closed">
...
<MermaidDiagram chart={`flowchart TD
    S["engine.can('alice', 'read', document)"] --> R1["resolveSubject('alice')"]
    R1 --> |"adapter.getSubjectRoles"| R2["assigned: ['viewer']"]
    R1 --> |"adapter.getSubjectAttributes"| R3["attributes: none"]
    R2 --> R4["resolveEffectiveRoles<br/>closes over inherits"]
    R4 --> P["rolesToPolicy()"]
    P --> P2["synthetic policy __rbac__<br/>algorithm allow-overrides"]
    P2 --> EV["evaluate(policies, request)"]
    EV --> D1["read on document<br/>first rbac rule matches<br/>ALLOW"]
    EV --> D2["delete on document<br/>no rule matches<br/>defaultEffect DENY"]`} />
...
<Callout tone="info" title="Attribute values are flat">
...
<Callout tone="warning" title="defaultEffect 'allow' is refused by default">
