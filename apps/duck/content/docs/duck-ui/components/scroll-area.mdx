...
Use `MotionScrollArea` for a smooth entrance animation powered by [motion](https://motion.dev). The container uses the shared `scaleIn` preset with a `springBouncy` transition on mount.
