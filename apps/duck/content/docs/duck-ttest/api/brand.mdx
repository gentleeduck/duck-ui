...
Edge case: `Equal<Brand<string, 'X'>, string>` is `false`, because branded types do not equal their base. This is by design but a frequent confusion. See [Core / Composing tests](/duck-ttest/core/composing-tests) for the brand-aware assertion pattern.
