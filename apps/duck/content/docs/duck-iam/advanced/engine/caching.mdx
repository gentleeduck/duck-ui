...
description: The five LRU caches, their keys and sizes, single-flight coalescing, every invalidation trigger and the exact order it clears things
section: Advanced
---
...
<MermaidDiagram chart={`flowchart TD
    A["loader called"] --> B{"LRU hit and not expired?"}
    B -- "yes" --> C["return cached value"]
    B -- "no" --> D{"in-flight promise for this key?"}
    D -- "yes" --> E["join it - no second adapter call"]
    D -- "no" --> F{"subject load and cap reached?"}
    F -- "yes" --> G["reject: subject load shed"]
    F -- "no" --> H["store pending promise in the slot"]
    H --> I["adapter call wrapped in adapterTimeoutMs"]
    I --> J{"row count over maxPolicies / maxRoles?"}
    J -- "yes" --> K["throw - fail closed"]
    J -- "no" --> L{"slot still holds THIS promise?"}
    L -- "yes" --> M["write the LRU entry"]
    L -- "no" --> N["discard - an invalidation landed mid-flight"]`} />
...
<MermaidDiagram chart={`flowchart LR
    IS["invalidateSubject(id)"] --> S1["subject cache: that key"]
    IS --> S2["in-flight subjects: that key"]
    IP["invalidatePolicies()"] --> P1["policy cache"]
    IP --> P2["merged cache"]
    IP --> P3["compiled table"]
    IR["invalidateRoles(roleId?)"] --> R1["role cache"]
    IR --> R2["RBAC policy cache"]
    IR --> R3["merged cache"]
    IR --> R4["compiled table"]
    IR --> R5["subjects holding roleId, or all when roleId is omitted"]
    IA["invalidate()"] --> ALL["every cache, every in-flight slot, compiled table"]`} />
...
<Callout tone="info" title="Short TTL is the zero-configuration answer">
