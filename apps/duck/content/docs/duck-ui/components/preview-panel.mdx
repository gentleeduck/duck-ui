...
Use `MotionPreviewPanel` for a smooth entrance animation powered by [motion](https://motion.dev). The panel uses a `scaleIn` entrance with a bouncy spring transition on mount, and its internal controls (buttons, separators, badges, tooltips) use the full motion stack.
