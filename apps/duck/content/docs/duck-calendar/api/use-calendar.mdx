...
`useCalendar` accepts a `UseCalendar.IUseCalendarConfig<TDate, M>` object:
...
| `selectDate` | `(date: TDate, options?: { shiftKey?: boolean }) => void` | Select a date. In `multi-range` mode, `shiftKey: true` splits an existing range longer than 5 days to exclude the clicked date; ignored in other modes |
