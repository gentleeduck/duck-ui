---
title: useUploader
description: The React hook for reading upload state and dispatching commands.
...
Note `failed` maps to the `error` **phase**, so when you check `item.phase`, use `'error'`.
