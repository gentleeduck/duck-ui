...
Edge case: `Not<boolean>` is `boolean`, because the distribution over `true | false` flips each member back to a union of both.
...
Edge case: every boolean operator here constrains both generics to `boolean`. Passing `boolean` (the union `true | false`) distributes, so the result is also `boolean`. Pin to a literal first if that is not what you want.
