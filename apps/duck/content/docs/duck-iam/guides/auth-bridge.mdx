...
description: duck-auth answers "who are they?", duck-iam answers "what may they do?". Wire the two with a projectToSubject function you own
section: Guides
---
...
<MermaidDiagram chart={`sequenceDiagram
    ...
    participant A as "auth (AuthEngine)"
    participant P as "projectToSubject"
    participant E as "engine (IamEngine)"
    C->>H: "DELETE /posts/123 with session cookie"
    H->>A: "auth.resolveSession({ headers })"
    A-->>H: "{ session, identity } or null"
    alt "null"
        H-->>C: "401 AUTH_UNAUTHENTICATED"
    else "resolved"
        H->>A: "auth.orgs.resolveMembership(orgId, identity.id)"
        A-->>H: "Org.Membership with roles"
        H->>P: "projectToSubject(identity, session, membership)"
        P-->>H: "IamRequest.ISubject"
        H->>E: "engine.authorize({ subject, action, resource, scope })"
        E-->>H: "IDecision or boolean"
        alt "denied"
            H-->>C: "403"
        else "allowed"
            H-->>C: "204"
...
<MermaidDiagram chart={`flowchart TD
    Q{"Where do role assignments live?"}
    Q -- "in the iam adapter" --> A1["engine.can(identity.id, action, resource, env, scope)"]
    Q -- "in duck-auth org memberships" --> A2["projectToSubject(...) then engine.authorize(request)"]
    A1 --> N1["Engine resolves roles from its own adapter and caches them"]
    A2 --> N2["You supply subject.roles; the engine skips subject resolution"]`} />
...
<Callout tone="warning" title="Attribute bags are bounded">
...
<Callout tone="warning" title="Never let the auth error code decide authorization">
...
<Callout tone="warning" title="`explain()` output is not safe to return to a client">
...
<Callout tone="warning" title="Org roles are silently truncated on the auth side">
