...
description: Exact shape of the permission map, its four key formats, escaping rules, partial maps, and the iamBuildPermissionKey helper
...
<MermaidDiagram chart={`flowchart TD
    A["iamBuildPermissionKey(action, resource, resourceId, scope)"] --> B{"scope !== undefined"}
    B -- "no" --> C{"resourceId !== undefined"}
    B -- "yes" --> D{"resourceId !== undefined"}
    C -- "no" --> E["action:resource"]
    C -- "yes" --> F["action:resource:resourceId"]
    D -- "no" --> G["@scope:action:resource"]
    D -- "yes" --> H["@scope:action:resource:resourceId"]
    E --> I["Each segment escaped if it holds a colon, a backslash, or a leading @"]
    F --> I
    G --> I
    H --> I`} />
...
The static return type depends on engine mode: `production` yields `Record<string, boolean>`, `development` yields the typed `IamClient.PermissionMap`. That is `AccessControl.ModePermissionMap` at work - see [engine modes](/duck-iam/advanced/engine/modes). The runtime object is identical in both modes.

## Reading a map

Reading goes through `iamPermissionGranted`, which requires the map's own `key` to hold the literal boolean `true`. Nothing validates the map - it is `JSON.parse` output whose type is erased at runtime - so a truthiness test would read `{"read:post": "false"}` as a grant. A check that does not match a key, or matches a key holding anything other than `true`, denies.

<MermaidDiagram chart={`flowchart TD
    A["can(action, resource, resourceId, scope)"] --> B["Build the key from the four arguments"]
    B --> C{"Key present in the map"}
    C -- "no" --> D["false - never batched, or wrong shape"]
    C -- "yes" --> E{"Value"}
    E -- "boolean true" --> F["true - allowed"]
    E -- "anything else" --> G["false - denied, or not a boolean at all"]
    D --> H["cannot() returns true for D and G alike"]
    G --> H`} />
