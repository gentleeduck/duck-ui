...
Use `MotionSlider` for a smooth entrance animation powered by [motion](https://motion.dev). The slider uses a `scaleIn` entrance with a `springBouncy` transition on mount.
