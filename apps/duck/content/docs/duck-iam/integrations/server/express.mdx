---
title: Express
description: The Express middleware, per-route guard, and admin router — signatures, extraction defaults, and error mapping for Express 4 and 5
...
<Steps>
<Step>Build the engine once per process</Step>
...
<Step>Put your auth layer in front</Step>
...
<Step>Optionally replace the 500 body</Step>
...
<Callout tone="warning" title="onError is not handed next, deliberately">
...
<Callout tone="info" title="Middleware and handler, not one or the other">
...
<Callout tone="warning" title="Return the actor, not true">
