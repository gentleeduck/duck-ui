...
Use `MotionInputOTP` for a `scaleIn` entrance animation with a bouncy spring transition on the entire OTP input, powered by [motion](https://motion.dev).
...
Same props as `InputOTP`. Adds a `scaleIn` entrance animation with a bouncy spring transition on the entire OTP container. Requires the `motion` package.
