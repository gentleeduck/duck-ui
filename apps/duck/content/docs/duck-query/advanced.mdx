---
title: advanced
description: Advanced Duck Query patterns. Covers interceptors, error handling, custom Axios instances, using without Duck Gen, and real-world integration tips.
section: Advanced
order: 0
---
## Interceptors
...
Pass an existing Axios instance instead of a config object. Useful for sharing one
...
- **GET and DELETE drop the body**: a `body` field on the request is ignored for GET,
  DELETE, HEAD, and OPTIONS.
- **`request` defaults to GET**: no `config.method` means `'GET'`.
- **Path type checking**: `client.post()` only accepts POST-capable paths. Posting to a
  GET-only route is a compile-time error.
- **Param encoding**: path params run through `encodeURIComponent`.
- **No slash normalization**: the built URL is `baseURL + path` with param substitution
  only; a `baseURL` ending in `/` combined with a path starting with `/` produces a double
  slash. Keep one side without a trailing/leading slash.
