---
title: Introduction
description: duck-iam is a type-safe RBAC + ABAC authorization engine for TypeScript, with one evaluation pipeline for roles and policies
...
<MermaidDiagram chart={`flowchart TD
    subgraph app["Your application"]
        SRV["Server route, middleware or guard"]
        CLI["Browser UI gate"]
    end

    subgraph eng["IamEngine"]
        RES["Resolve subject: roles, scoped roles, attributes"]
        LOAD["Load catalog: your policies + generated RBAC policy"]
        EV["Evaluate: per-policy algorithm, then policyCombine"]
        HOOK["Hooks: beforeEvaluate, afterEvaluate, onDeny, onError, onPolicyError, onMetrics, onMutation"]
    end

    subgraph ops["Storage and operability"]
        AD["Adapter: memory, file, prisma, drizzle, redis, http"]
        INV["Invalidator: redis pub/sub"]
        MET["Metrics aggregator: p50, p95, p99"]
    end
...
<MermaidDiagram chart={`flowchart TD
    Q["What are you doing?"] --> A["Setting it up"]
    Q --> B["Modelling permissions"]
    Q --> C["Wiring a framework"]
    Q --> D["Operating it"]
    Q --> E["Debugging a decision"]

    A --> A1["/duck-iam/installation"]
    A --> A2["/duck-iam/guides (quick start)"]
    A --> A3["/duck-iam/course (8 chapters)"]

    B --> B1["/duck-iam/core/roles"]
    B --> B2["/duck-iam/core/policies"]
    B --> B3["/duck-iam/core/evaluation"]

    C --> C1["/duck-iam/integrations/server"]
    C --> C2["/duck-iam/integrations/client"]
    C --> C3["/duck-iam/integrations/adapters"]

    D --> D1["/duck-iam/guides/production"]
    D --> D2["/duck-iam/advanced/engine/caching"]
    D --> D3["/duck-iam/integrations/observability/metrics"]

    E --> E1["/duck-iam/advanced/explain"]
    E --> E2["/duck-iam/guides/troubleshooting"]
    E --> E3["/duck-iam/advanced/devtools"]`} />
...
<Callout tone="warning" title="mode defaults to production since 5.9.0">
...
<Accordion type="multiple" collapsible className="w-full">
  ...
  <AccordionItem value="rbac-or-abac">
    <AccordionTrigger>Is it RBAC or ABAC?</AccordionTrigger>
    <AccordionContent className="text-muted-foreground">
  ...
  <AccordionItem value="can-it-allow-by-accident">
    <AccordionTrigger>Can it accidentally allow something?</AccordionTrigger>
    <AccordionContent className="text-muted-foreground">
