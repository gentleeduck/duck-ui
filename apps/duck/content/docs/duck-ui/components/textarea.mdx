...
Use `MotionTextarea` for a smooth entrance animation powered by [motion](https://motion.dev). The textarea uses a `scaleIn` entrance with a `springBouncy` transition on mount.
