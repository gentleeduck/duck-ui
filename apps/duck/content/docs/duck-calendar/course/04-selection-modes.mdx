...
`data-range-middle` (and `data-range-start` / `data-range-end`) reflect the committed selection only - there's no built-in hover preview between the first and second click. Track hover state yourself and compute a candidate range if you want that effect.
