---
title: Compiled table
description: How production mode compiles roles and policies into typed arrays - wildcard buckets, role bitmasks, the 32-role cap, residual policies and the lookup path
section: Advanced
---
...
<MermaidDiagram chart={`flowchart TD
    R["role permissions"] --> RW{"wildcard action or resource?"}
    RW -- "yes" --> RES1["rbacResidual - rolesToPolicy over the leftovers"]
    RW -- "no" --> RS{"conditions or a literal scope?"}
    RS -- "no" --> BAKE["allow bitmask: OR in every holder role's bit"]
    RS -- "yes" --> RDYN["rbacDynamic cell group: roleMask, scope, conditions"]
    P["ABAC policies"] --> PC{"any wildcard rule, or a wildcarded target?"}
    PC -- "yes" --> RES2["residualPolicies - evaluated per request"]
    PC -- "no" --> FLAT["flat policies"]
    FLAT --> CELL{"per rule, per action-resource cell"}
    CELL -- "has conditions, or targets.roles" --> DYN["kind = DYNAMIC, dynamic[idx] gets a policy group"]
    CELL -- "unconditional allow" --> CA["kind = CONST_ALLOW"]
    CELL -- "unconditional deny" --> CD["kind = CONST_DENY"]
    CA -- "same cell also gets a deny" --> DYN
    BAKE --> T["CompiledTable"]
    RDYN --> T
    RES1 --> T
    CA --> T
    CD --> T
...
<Callout tone="info" title="Over 32 roles is a throughput cliff, not a denial">
...
<MermaidDiagram chart={`flowchart TD
    L["lookup(table, mask, action, resource, req)"] --> A["abacFlatVote"]
    L --> B["rbacVote"]
    L --> C["residualPolicies loop - evaluatePolicyFast each"]
    A --> A1{"hasFlatSource, known dims, touched?"}
    A1 -- "no" --> AN["abstain"]
    A1 -- "yes" --> A2{"kind"}
    A2 -- "CONST_ALLOW" --> AT["true"]
    A2 -- "CONST_DENY" --> AF["false"]
    A2 -- "DYNAMIC" --> AD["evaluateDynamicCell over the policy groups"]
    B --> B1{"mask AND allow[idx] nonzero?"}
    B1 -- "yes" --> BT["true"]
    B1 -- "no" --> B2["scan rbacDynamic groups"]
    B2 --> B3["fall back to rbacResidual"]
    B3 --> B4{"any role grants this cell at all?"}
    B4 -- "yes" --> BD["defaultEffect"]
    B4 -- "no" --> BN["abstain"]
    AT --> Z["collect non-abstaining votes, then combine"]
    AF --> Z
    AD --> Z
    BT --> Z
    BD --> Z
...
<Callout tone="warning" title="RBAC's three sources are one vote, never three">
...
<MermaidDiagram chart={`flowchart TD
    R["rule with wildcards"] --> Q{"which side is literal?"}
    Q -- "neither" --> WB["wildcardBoth - always scanned"]
    Q -- "action literal, resource wildcard" --> BA["byActionWildcardResource, keyed per literal action"]
    Q -- "resource literal, action wildcard" --> BR["byResourceWildcardAction, keyed per literal resource"]
    Q -- "both literal" --> BL["byActionResource, keyed on the exact pair"]
    REQ["request: action, resource"] --> LK["look up the two targeted buckets by key"]
    LK --> BA
    LK --> BR
    LK --> WB`} />
