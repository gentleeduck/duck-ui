...
- The engine is **date-library agnostic**, so you can swap adapters without touching rendering code.
...
- The adapter is a plain object, with no classes and no inheritance.
