...
<MermaidDiagram chart={`graph TD
  ...
  Collapsible --> primitives["@gentleduck/primitives\n/collapsible"]:::internal
  Collapsible --> libs["@gentleduck/libs"]:::internal
  Collapsible --> motion["@gentleduck/motion"]:::internal

  primitives --> |"provides"| OpenState["Open/Close\nState"]
  primitives --> |"provides"| A11y["ARIA\nAttributes"]
  Collapsible --> |"adds"| Trigger["CollapsibleTrigger"]
  Collapsible --> |"adds"| Content["CollapsibleContent"]
  motion --> |"provides"| HeightAnim["Height Animation"]
...
<ComponentPreview name="collapsible-3" description="A collapsible with an active trigger style when expanded." />
...
<ComponentPreview
  name="collapsible-4"
  description="A collapsible with motion-powered height animation."
/>
...
Animates height from 0 to auto with a blur and opacity fade. Requires the `motion` package.

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `...props` | `Omit<React.HTMLAttributes<HTMLDivElement>, 'onDrag' \| 'onDragStart' \| 'onDragEnd' \| 'onAnimationStart'>` | - | Additional props to spread to the underlying `section` element. This is a separate component from `CollapsibleContent`, not a wrapper around it, so it does not accept `forceMount`. |
