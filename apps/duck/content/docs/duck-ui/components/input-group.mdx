...
Use `MotionInputGroup` for a `scaleIn` entrance animation with a bouncy spring transition, powered by [motion](https://motion.dev). Pass `index` for stagger delay when using multiple groups.
...
Same props as `InputGroup` plus an optional `index` prop for stagger delay (50ms per index). Adds a `scaleIn` entrance animation with a bouncy spring transition. Requires the `motion` package.
