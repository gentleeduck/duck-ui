...
`paused` phase with `file: undefined`, because browser `File` objects can't be serialized. Progress and
...
after dispatching, since `resume` is a no-op if the file wasn't attached:
