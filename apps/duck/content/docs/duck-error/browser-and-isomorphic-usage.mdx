...
A server's `err.toJSON()` output travels to the client as plain JSON — a client-side `fetch`
...
kit itself — `createErrorKit` should still only run once, wherever the errors are actually
constructed) through whatever your project already uses to share types between client and
server — a shared `types.ts` module, a generated OpenAPI/tRPC client, and so on. This package
