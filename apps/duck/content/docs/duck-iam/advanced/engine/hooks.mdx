---
title: Hooks
description: The six engine hooks - exact signatures, firing order, which modes they run in, and what happens when one of them throws
section: Advanced
---
...
<MermaidDiagram chart={`sequenceDiagram
    ...
    C->>E: can / check / authorize
    E->>H: beforeEvaluate
    H-->>E: possibly modified request
    E->>E: ensureEnvNow defaults environment.now
    E->>E: compiled lookup, plus the interpreter in development
    E->>H: onPolicyError per broken policy
    E->>H: afterEvaluate
    E->>H: onDeny - deny only
    E->>H: onMetrics
    E-->>C: decision or boolean`} />
...
<MermaidDiagram chart={`flowchart TD
    A["a hook throws"] --> B{"which hook?"}
    B -- "beforeEvaluate" --> C["caught by authorize: onError fires, fail-closed deny returned"]
    B -- "afterEvaluate" --> D["swallowed, logged to console.error, decision unchanged"]
    B -- "onDeny" --> D
    B -- "onError" --> D
    B -- "onMetrics" --> D
    B -- "onMutation" --> F["swallowed, logged; the write it reports already landed"]
    B -- "onPolicyError" --> E["propagates into the evaluator's own catch for that policy"]`} />
...
<Callout tone="warning" title="Do not throw from a hook to block a request">
...
<Callout tone="warning" title="An unwired onPolicyError hides broken rows">
