...
<ComponentPreview
  name="hover-card-5"
  description="A hover card with an arrow pointing at the trigger."
/>
...
| `placement` | `'top' \| 'right' \| 'bottom' \| 'left'` | `'top'` | Accepted but has no effect on the base `HoverCard`/`HoverCardContent` pairing; only takes effect with `MotionHoverCard`/`MotionHoverCardContent`, which read it via context to pick the entrance direction |
...
| `...props` | `React.ComponentPropsWithoutRef<typeof HoverCardPrimitive.Trigger> & Variants.VariantProps<typeof buttonVariants>` | - | Additional props inherited from `HoverCard.Trigger`. Always renders a native `a` element; there is no `asChild`/`Slot` support. |
...
| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `placement` | `'top' \| 'right' \| 'bottom' \| 'left'` | `'top'` | Provided via context to `MotionHoverCardContent`, which uses it as the default entrance/exit direction when its own `side` prop is unset. This is where `placement` actually takes effect. |
