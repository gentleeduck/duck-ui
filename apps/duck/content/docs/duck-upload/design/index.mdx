...
description: The architecture choices behind @gentleduck/upload and why they were made.
section: Concepts
order: 0
---
...
There is one way to change state: `dispatch(command)`. There is one way to observe it:
...
So UI code reads `item.result.url` with no cast, because TypeScript already knows the shape.
...
fixed `useSyncExternalStore` tearing, so React can compare snapshots by reference and re-render
