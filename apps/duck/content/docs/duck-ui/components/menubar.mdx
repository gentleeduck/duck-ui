...
Use `MotionMenubarContent` for a `scaleIn` enter animation with a bouncy spring transition, powered by [motion](https://motion.dev). Exit uses the primitive's built-in CSS animation.
...
Same props as `MenubarContent`. Adds a `scaleIn` enter animation with a bouncy spring transition via motion. Exit uses the primitive's CSS animation. Requires the `motion` package.
