...
description: Nested condition groups with and(), or(), not(), and whenAny() - evaluation order, short-circuiting, the depth limit, and empty-group semantics
section: Core
---
...
<Callout tone="warning" title="not() is NOR, not a per-child negation">
...
<MermaidDiagram chart={`flowchart TD
    G["group at depth d"] --> D{"d >= 10?"}
    D -- "yes" --> FC["throw IamConditionGroupError('depth')"]
    D -- "no" --> K{"group kind"}
    K -- "all" --> A["child 1, child 2, ... in insertion order"]
    A --> A1{"child false?"}
    A1 -- "yes" --> AF["stop, return false"]
    A1 -- "no, list exhausted" --> AT["return true"]
    K -- "any" --> B["child 1, child 2, ... in insertion order"]
    B --> B1{"child true?"}
    B1 -- "yes" --> BT["stop, return true"]
    B1 -- "no, list exhausted" --> BF["return false"]
    K -- "none" --> C["child 1, child 2, ... in insertion order"]
    C --> C1{"child true?"}
    C1 -- "yes" --> CF["stop, return false"]
    C1 -- "no, list exhausted" --> CT["return true"]
    K -- "empty object" --> U["return true (no conditions)"]
    K -- "unrecognised keys" --> X["throw IamConditionGroupError('unknown-keys')"]`} />
...
<MermaidDiagram chart={`flowchart LR
    L1["chain of 10 groups: root all plus 9 nested"] --> OK["evaluates normally; build() accepts"]
    L11["11 groups or deeper"] --> HARD["build() throws LIMIT_EXCEEDED; evalConditionGroup throws IamConditionGroupError"]`} />
...
<Callout tone="warning" title="An empty whenAny() is a rule that matches nothing">
