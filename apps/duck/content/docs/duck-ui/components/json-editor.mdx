...
Use `MotionJsonTextareaField` for a `scaleIn` entrance animation with a bouncy spring transition, powered by [motion](https://motion.dev).
...
Same props as `JsonTextareaField`. Adds a `scaleIn` entrance animation with a bouncy spring transition via motion. Requires the `motion` package.
