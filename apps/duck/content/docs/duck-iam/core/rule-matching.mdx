---
title: Rule matching
description: How the engine decides whether a rule fires - action and resource patterns, condition groups, field resolution, $-references, and every condition operator with its edge-case semantics.
section: Core
---
...
flowchart TD
  REQ["Access request"] --> A{"Action gate"}
  A -- "pattern matches" --> R{"Resource gate"}
  A -- "no match" --> SKIP["Rule skipped"]
  R -- "pattern matches" --> C{"Condition gate"}
  R -- "no match" --> SKIP
  C -- "group evaluates true" --> FIRE["Rule fires with its effect"]
  C -- "false or depth exceeded" --> SKIP
`} />
...
<Callout tone="warning" title="Hierarchy is opt-in per pattern">
...
<MermaidDiagram chart={`
flowchart LR
  P["field path"] --> ROOT{"first segment"}
  ROOT -- "subject" --> S["request.subject"]
  ROOT -- "resource" --> RS["request.resource"]
  ROOT -- "environment" --> E["request.environment"]
  ROOT -- "action" --> AC["request.action"]
  ROOT -- "scope" --> SC["request.scope or null"]
  ROOT -- "anything else" --> N["null"]
  S --> WALK["walk remaining segments"]
  RS --> WALK
  E --> WALK
  WALK -- "segment missing" --> N
  WALK -- "blocked segment" --> N
  WALK -- "found" --> V["value"]
`} />
...
<Callout tone="warning" title="Negative operators default to true">
