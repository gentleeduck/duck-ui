...
3. **Uses external state machines**. Ark UI offloads to xstate/zag.js, adding roughly
   15 KB of runtime that isn't counted in the component size.
...
- **Calendar**: the only headless library with a multi-calendar-system primitive.
...
- **No compound pattern overhead**: its render-prop API is lighter for basic cases.
- **Google backing**: part of the MUI ecosystem.

### Ark UI wins on

- **State machines**: xstate/zag.js gives deterministic transitions in complex interactions.
- **Color Picker**: ships a color picker primitive others lack.
- **Combobox**: a dedicated combobox primitive (gentleduck uses Command instead).

### Headless UI wins on

- **Smallest total size**: about 35 KB for the whole library, with only 10 components.
- **Tailwind integration**: a class-based API designed for Tailwind CSS.
...
Install 5 Radix primitives and you carry about 25 KB of duplicated internals. With gentleduck, the
...
- **gentleduck**: sizes from `packages/duck-primitives/dist/`, concatenating all `.js` files per primitive directory, then `gzip -c | wc -c`.
