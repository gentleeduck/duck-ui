---
title: aspect-ratio
description: Displays content within a desired ratio.
component: true
section: Layout
order: 10
---
<ComponentPreview
  name="aspect-ratio-1"
  title="Aspect Ratio"
  description="A component that displays an image with a 16:9 aspect ratio."
/>

## Philosophy

Responsive media needs dimensional constraints. AspectRatio solves the CSS aspect-ratio problem with a clean component API, preventing layout shifts when images and videos load. The `ratio` prop is the only knob you need  -  everything else is handled by the container's CSS.

## How It's Built

<MermaidDiagram chart={`graph TD
  AspectRatio[AspectRatio]
  AspectRatio --> primitives["@gentleduck/primitives\n/slot"]:::internal
  AspectRatio --> libs["@gentleduck/libs"]:::internal
  primitives --> |"provides"| SlotRendering["Slot Rendering\n(wraps child)"]
  AspectRatio --> |"manages"| PaddingTrick["CSS Padding\nAspect Ratio"]
  AspectRatio --> |"adds"| ResponsiveMedia["Responsive Media\nContainment"]
  classDef internal fill:#2dd4bf,stroke:#14b8a6,color:#fff
`} />

## Installation

<Tabs defaultValue="cli">

<TabsList>
  <TabsTrigger value="cli">CLI</TabsTrigger>
  <TabsTrigger value="manual">Manual</TabsTrigger>
</TabsList>
<TabsContent value="cli">

```bash
npx @gentleduck/cli add aspect-ratio
```

</TabsContent>

<TabsContent value="manual">

<Steps>

<Step>Install the following dependencies:</Step>

```bash
npm install @gentleduck/primitives @gentleduck/libs
```

<Step>Copy and paste the following code into your project.</Step>

<ComponentSource path="packages/registry-ui/src/aspect-ratio" />

<Step>Update the import paths to match your project setup.</Step>

</Steps>

</TabsContent>

</Tabs>

## Usage

```tsx
import Image from "next/image"

import { AspectRatio } from "@/components/ui/aspect-ratio"
```

```tsx
<div className="w-[450px]">
  <AspectRatio ratio={16 / 9}>
    <Image src="..." alt="Image" className="rounded-md object-cover" />
  </AspectRatio>
</div>
```

## RTL Support

Direction is resolved through the shared primitives direction module. Use a local `dir="rtl"` override when the component exposes it, or set `DirectionProvider` at app/root level for global RTL/LTR behavior.

<ComponentPreview
  name="aspect-ratio-2"
  description="An aspect ratio with RTL support."
/>

## Motion

<Callout
  icon={<TriangleAlert />}
  title="Alpha: Motion Compositions"
  tone="warning">
Motion components work standalone, but some compositions may behave unexpectedly: this is still under active development. If you find a broken composition, please [file an issue](https://github.com/gentleeduck/gentleduck/issues).
</Callout>

Use `MotionAspectRatio` for a smooth scale + blur entrance animation powered by [motion](https://motion.dev). The content scales from 0.95 and unblurs on mount.

<ComponentPreview
  name="aspect-ratio-3"
  description="An aspect ratio container with motion-powered entrance animation."
/>

<Callout icon={<Zap />}>
Requires the `motion` package. Use `MotionAspectRatio` instead of `AspectRatio`. Same props. Note: `MotionAspectRatio` adds an animated `div` wrapper around `AspectRatio`; the inner `Slot`/`asChild` behavior is unchanged.
</Callout>

## API Reference

### AspectRatio

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `dir` | `'ltr' \| 'rtl'` | - | Text direction override. Resolved via `useDirection` (`dir` prop -> `DirectionProvider` -> `'ltr'`). |
| `ratio` | `number` | (required) | Aspect ratio of the container, e.g. `16/9`, `4/3` |
| `className` | `string` | `--` | Additional class names to apply to the container |
| `style` | `React.CSSProperties` | `--` | Inline styles applied to the container (aspect-ratio is merged automatically) |
| `children` | `React.ReactNode` | `--` | Child element to render inside the aspect-ratio container |
| `...props` | `React.ComponentPropsWithoutRef<typeof Slot>` | - | Additional props inherited from `Slot`. |

*Uses a `Slot` wrapper, so the child element receives the aspect-ratio styles.*

### MotionAspectRatio

Wraps `AspectRatio` in an animated `div` that scales from 0.95 with 8px blur fade on mount, using the `scaleIn` preset and a bouncy spring transition (stiffness 500, damping 28). The wrapped `AspectRatio` still renders its child via `Slot` internally. Requires the `motion` package.

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `...props` | `AspectRatioProps` | - | All props from `AspectRatio` are supported |
