...
description: engine.explain() returns a complete, non-short-circuiting trace of every policy, rule, and condition. Field-by-field reference with a real sample trace
section: Advanced
---
...
<Callout tone="warning" title="The trace is privileged data">
...
<MermaidDiagram chart={`flowchart TD
    A["engine.explain(subjectId, action, resource, environment, scope)"] --> B["resolve subject, enrich scoped roles"]
    B --> C["beforeEvaluate hook, then ensureEnvNow"]
    C --> D["loadAllPolicies: rbac policy first, then stored policies"]
    D --> E["explainEvaluation(policies, request, defaultEffect, subjectInfo, combine)"]
    E --> F["tracePolicy for every policy, no short-circuit"]
    F --> G{"policyTargetsMatch?"}
    G -- "no" --> H["IPolicyTrace with targetMatch false, rules empty, result defaultEffect"]
    G -- "yes" --> I["traceRule for every rule"]
    I --> J["matchesAction and matchesResource"]
    I --> K["traceGroup on rule.conditions, depth capped at 10"]
    K --> L["traceLeaf: resolve actual, iamResolveConditionValue expected, iamEvaluateOperator"]
    J --> M["applyCombiner on matched rule traces"]
    K --> M
    M --> N["IPolicyTrace with result, reason, decidingRuleId, decidingRule"]
    H --> O["decideFinal across all traces under the combine mode"]
    N --> O
    O --> P["AccessControl.IDecision"]
    P --> Q["buildSummary -> summary string"]
    Q --> R["Explain.IResult"]`} />
...
<MermaidDiagram chart={`classDiagram
    ...
    IResult "1" --> "many" IPolicyTrace : policies
    IPolicyTrace "1" --> "many" IRuleTrace : rules
    IRuleTrace "1" --> "1" IGroupTrace : conditions
    IGroupTrace "1" --> "many" IGroupTrace : children
    IGroupTrace "1" --> "many" ILeafTrace : children`} />
...
<Callout tone="warning" title="INFO-B: the summary is unescaped">
