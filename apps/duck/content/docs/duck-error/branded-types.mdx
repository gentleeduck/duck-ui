---
title: branded types
description: How detail() and fault() brand a plain status number so the compiler can compute each code's metadata requirement — Carries, Fault, Bare, Args, and Faults in depth.
---

## The problem a brand solves

A registry's values are `number`. If `detail`/`fault` just returned `number`, the type system
would have nowhere to record "this particular 429 also carries `{ retryAfter: number }}`" --
...
`__carries` and `__fault` are never actually present on the value at runtime -- `detail(429)`
...
carries nothing, the same as writing the plain number `429` -- there is no accidental "carries
anything" state.
...
whether `Meta<R, C>` happens to be non-empty -- a bare code's `Meta` resolves to
...
identically at runtime -- this type is the entire point of the distinction. A concrete use: a
