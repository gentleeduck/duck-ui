---
title: NestJS
description: The @IamAuthorize decorator, the canActivate guard body, the engine provider, and gated admin operations for NestJS controllers
...
<Steps>
<Step>Register the engine as a provider</Step>
...
<Step>Mark the handlers that need a check</Step>
...
<Step>Map guard denials to a response</Step>
...
<MermaidDiagram chart={`sequenceDiagram
    ...
    participant G as iamNestAccessGuard
    participant M as Handler metadata
    participant E as IamEngine
    participant H as Controller method
    C->>A: DELETE /posts/42
    A->>A: populate req.user
    A->>G: canActivate(context)
    G->>M: getHandlerMeta(context.getHandler())
    alt no IamAuthorize metadata
        M-->>G: not present
        G->>H: return true, handler runs unchecked
    else present but unreadable
        M-->>G: null, a string, a number, a bad field type
        G-->>C: onError(Error), default false
    else metadata found
        M-->>G: action, resource, scope, infer
        G->>G: getUserId(request)
        alt no subject
            G-->>C: return false, Nest sends 403
        else subject found
            G->>G: getResourceAttributes and getEnvironment
            G->>E: can(id, action, resource, env, scope)
            E-->>G: true or false
            alt allowed
                G->>H: handler runs
                H-->>C: 200
...
<Callout tone="warning" title="A handler with no decorator is allowed">
...
<Callout tone="warning" title="@IamAuthorize({}) always denies">
...
<Callout tone="warning" title="onError decides the verdict">
...
<Callout tone="warning" title="CSRF check is on by default, including on reads">
...
<Callout tone="info" title="Scoped revoke is not on the operations">
