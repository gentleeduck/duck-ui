...
<Step>Make sure you have the [`Label`](/duck-ui/components/label) component installed in your project.</Step>
...
<ComponentPreview
  name="checkbox-10"
  description="Checkboxes with motion-powered staggered entrance."
/>
...
Wraps `CheckboxWithLabel` with a staggered `scaleIn` entrance (fade, scale, and blur). Uses `MotionCheckbox` internally. Requires the `motion` package.
