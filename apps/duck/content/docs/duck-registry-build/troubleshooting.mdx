...
Everything must satisfy ``registry:${string}``.
  ...
  <AccordionItem value="ts-config-loading">
    <AccordionTrigger>TS config file fails to load</AccordionTrigger>
    <AccordionContent className="text-muted-foreground">

The loader tries a native `import()` first, then falls back to `jiti` so TS/JS
configs load in Bun and in plain Node, not just Bun. If both attempts fail, the
error reports each stage. Use a JSON config only if you need to rule the loader
out entirely; otherwise keep custom logic in TS/JS and let the fallback handle it.
