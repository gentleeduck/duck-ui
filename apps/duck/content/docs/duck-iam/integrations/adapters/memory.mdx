...
description: Map-backed in-process adapter for tests, prototypes, and CI - zero dependencies, full interface coverage, and no persistence at all
...
<MermaidDiagram chart={`erDiagram
    POLICIES {
        string key "policy.id"
        object value "AccessControl.IPolicy"
    }
    ROLES {
        string key "role.id"
        object value "AccessControl.IRole"
    }
    ASSIGNMENTS {
        string key "subjectId"
        array value "Array of role and optional scope"
    }
    ATTRIBUTES {
        string key "subjectId"
        object value "IamPrimitives.Attributes"
    }
    ASSIGNMENTS ||--o{ ROLES : "entry.role references role.id"
    ATTRIBUTES ||--|| ASSIGNMENTS : "same subjectId"`} />
...
<Callout tone="warning" title="Never ship IamMemoryAdapter to production">
