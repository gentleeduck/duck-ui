...
narrows the type and gives you `item.progress`, `item.intent`, and friends.
...
State-derived public events emit from one place after transitions, never from inside individual
