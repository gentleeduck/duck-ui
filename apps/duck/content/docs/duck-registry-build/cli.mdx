...
Starting from the discovery root, the loader checks each filename below, then
walks up one parent directory at a time and checks again until it finds a match
