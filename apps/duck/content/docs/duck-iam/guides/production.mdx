...
description: SRE playbook for duck-iam - topology, cache TTLs, multi-node invalidation, fail-closed defaults, hooks, adapter tuning, metrics, and a pre-flight checklist
section: Guides
order: 99
---
...
<Callout tone="warning" title="Read SECURITY.md alongside this guide">
...
<MermaidDiagram chart={`flowchart TD
    LB["Load balancer"] --> P1["Pod A: app + IamEngine"]
    LB --> P2["Pod B: app + IamEngine"]
    LB --> P3["Pod C: app + IamEngine"]
    subgraph Store["Policy + role store"]
      DB["Prisma / Drizzle / Redis / HTTP adapter"]
    end
    P1 --> DB
    P2 --> DB
    P3 --> DB
    subgraph Bus["Invalidation bus"]
      R["Redis pub/sub - channel duck-iam:invalidate"]
    end
    P1 <--> R
    P2 <--> R
    P3 <--> R
    P1 --> M["onMetrics -> aggregator -> /metrics"]
    P1 --> HZ["healthCheck() -> /healthz"]
    ADM["Admin UI"] --> P1`} />
...
<Callout tone="warning" title="Production mode caps the catalog at 32 roles">
...
<Callout tone="warning" title="`healthCheck()` is not invalidation liveness">
...
<Callout tone="warning" title="SECURITY - multi-tenant Redis">
...
<Callout tone="warning" title="SECURITY - identity comes from a verified source">
...
<Callout tone="warning" title="SECURITY - opt out of CSRF only for bearer/mTLS APIs">
...
<Callout tone="warning" title="SECURITY - `defaultEffect: 'allow'` is a guard, not a default">
...
<Callout tone="info" title="Audit logging in production mode needs onMetrics, not onDeny">
...
<Callout tone="warning" title="SECURITY - set `allowedHosts` on the HTTP adapter">
