...
description: createIam() turns as const arrays of actions, resources, scopes and roles into union types that constrain every builder in the package
section: Advanced
---
...
<Callout tone="warning" title="A typo in an untyped permission is a silent deny">
...
<MermaidDiagram chart={`flowchart TD
    IN["createIam with actions, resources, scopes, roles, context"] --> CONST{"Declared as const?"}
    CONST -- "no" --> WIDE["TActions widens to plain string array"]
    WIDE --> NOCHECK["Every builder argument accepts any string"]
    CONST -- "yes" --> TUPLE["Literal tuple of 'create', 'read', 'update', 'delete'"]
    TUPLE --> UNION["TActions indexed by number becomes the action union"]
    UNION --> CFG["IamConfig.IAccessConfig carries TAction, TResource, TRole, TScope, TContext"]
    CFG --> B1["defineRole, definePolicy, defineRule, when"]
    CFG --> B2["createEngine, checks"]
    B1 --> OK["grant with 'read' and 'post' compiles"]
    B1 --> ERR["grant with 'raed' is a compile error"]`} />
...
<MermaidDiagram chart={`flowchart LR
    CFG["access"] --> DATA["Readonly data"]
    CFG --> BUILD["Builders"]
    CFG --> RUN["Runtime"]
    DATA --> D1["actions"]
    DATA --> D2["resources"]
    DATA --> D3["scopes, defaults to empty"]
    DATA --> D4["roles, defaults to empty"]
    BUILD --> B1["defineRole(id)"]
    BUILD --> B2["definePolicy(id)"]
    BUILD --> B3["defineRule(id)"]
    BUILD --> B4["when()"]
    RUN --> R1["createEngine(config)"]
    RUN --> R2["checks(arr)"]
    RUN --> R3["validateRoles(roles)"]
    RUN --> R4["validatePolicy(input)"]`} />
