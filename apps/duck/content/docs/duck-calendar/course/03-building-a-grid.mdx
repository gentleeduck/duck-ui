...
| `locale` | `Calendar.ICalendarLocaleConfig` | - | Locale settings (see below) |
...
`buildCalendarMonth` is pure, with no state. The next lesson uses `useCalendar` to add interactivity.
