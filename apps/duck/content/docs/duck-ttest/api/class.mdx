...
Edge case: like `ParametersSafe`, overloaded constructors collapse to the last overload, matching the built-in behavior.
...
Edge case: arrow functions and plain functions are not constructors, so `IsClass<() => void>` is `false`. Use [`IsFunction`](/duck-ttest/api/guard) or `ReturnTypeSafe` for those.
