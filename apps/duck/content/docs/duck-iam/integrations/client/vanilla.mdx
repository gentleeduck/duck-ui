---
title: Vanilla JS client
description: IamAccessClient - a dependency-free permission client with subscribe, merge, fromServer, and map-walking helpers for any JS runtime
...
<MermaidDiagram chart={`stateDiagram-v2
    [*] --> Empty: new IamAccessClient with no argument
    [*] --> Loaded: new IamAccessClient with a map
    Empty --> Loaded: update replaces the map
    Empty --> Reading: every check denies
    Loaded --> Loaded: update replaces
    Loaded --> Loaded: merge shallow-merges
    Loaded --> Reading: can, cannot, allowedActions, hasAnyOn
    Reading --> Loaded: synchronous, no mutation
    Loaded --> Notifying: update and merge notify listeners
    Notifying --> Loaded: a throwing listener is logged, the rest still run`} />
...
<Callout tone="warning" title="Handle the rejection">
...
<Callout tone="info" title="These are summaries, not checks">
...
<Callout tone="warning" title="Never build the client at module scope on a server">
