...
Use `MotionItem` and `MotionItemGroup` for staggered `scaleIn` entrance animations powered by [motion](https://motion.dev). Pass `index` to `MotionItem` for stagger delay.
...
Same props as `Item` plus an optional `index` prop for stagger delay (40ms per index). Adds a `scaleIn` entrance animation with a bouncy spring transition. Requires the `motion` package.

### MotionItemGroup

Same props as `ItemGroup`. Adds a `scaleIn` entrance animation with a bouncy spring transition. Requires the `motion` package.
