...
Use `MotionResizablePanelGroup` for a smooth entrance animation powered by [motion](https://motion.dev). The panel group uses a `scaleIn` entrance with a `springBouncy` transition on mount.
