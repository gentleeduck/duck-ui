...
Implement the trait for any other destination, whether that's a network service, a database, or anything else.
...
Later calls reuse the cached ID, a simple integer lookup. A `thread_local!` cache avoids
