...
export function DialogContentPrimitive({
  ...
  return (
    <dialog
      ref={ref}
      id={id}
      popover={modal ? undefined : 'auto'}
      className={cn(AnimVariants({ overlay: overlay }), AnimDialogVariants({ animation: animation }), className)}
      {...prop}>
...
function DialogContent({ children, className, ...props }: DialogContentProps): React.JSX.Element {
  return (
    <DialogContentPrimitive className={cn(AnimDialogModalVariants(), className)} {...props}>
      {children}
