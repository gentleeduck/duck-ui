...
description: Put the engine behind an HTTP API - middleware, per-route guards, resource loading, per-request scope, the admin router, and a permissions endpoint
section: Course
order: 6
---
...
<MermaidDiagram chart={`sequenceDiagram
    ...
    participant D as "Adapter + DB"
    participant H as "Route handler"
    C->>A: "PATCH /documents/doc-1"
    A->>A: "verify session cookie"
    A->>G: "next(), req.user = alice"
    G->>G: "getUserId, getEnvironment"
    G->>E: "can(alice, update, document doc-1)"
    E->>D: "resolveSubject(alice)"
    D-->>E: "roles, scopedRoles, attributes"
    E->>E: "beforeEvaluate hook loads the document row"
    E->>E: "evaluate __rbac__ then document-ownership"
    E-->>G: "false"
    G-->>C: "403 Forbidden"`} />
...
<Callout tone="warning" title="Identity comes from your auth layer, never from a header">
...
<Callout tone="warning" title="A field the host never set reads as a non-match, not an error">
...
<Callout tone="info" title="The default resource type is the raw path segment">
...
<Steps>
  <Step>
    **Install and mount authentication first**
    ...
    <PackageManagerTabs bun="bun add express" npm="npm i express" pnpm="pnpm add express" yarn="yarn add express" />
...
<MermaidDiagram chart={`flowchart TD
    A["Where does the check belong?"] --> B{"Does the scope change per request?"}
    B -- "no" --> C{"One route or many?"}
    C -- "one" --> D["iamGuard(engine, action, resource)"]
    C -- "many, uniform" --> E["iamAccessMiddleware(engine, opts)"]
    B -- "yes" --> F{"Is the scope in the path or the session?"}
    F -- "path or session" --> G["iamAccessMiddleware with getScope"]
    F -- "derived from the body" --> H["call engine.can() inside the handler"]
    D --> I["403 on deny, 401 with no user"]
    E --> I
    G --> I
    H --> J["you write the response"]`} />
