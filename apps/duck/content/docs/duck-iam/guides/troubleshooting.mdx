...
description: Real error messages from the source paired with the cause and the fix, plus how to diagnose a wrong decision that throws nothing
section: Guides
---
...
<MermaidDiagram
  chart={`
flowchart TD
  A["Unexpected result"] --> B{"Did something throw?"}
  B -- "yes" --> C{"Which tag?"}
  C -- "iam:engine" --> D["Config or input guard"]
  C -- "iam:builder" --> E["Validator issue codes"]
  C -- "iam:compiled" --> F["32-role production cap"]
  C -- "adapter tag" --> G["Connectivity or row shape"]
  B -- "no" --> H{"mode development?"}
  H -- "no" --> I["Reproduce on a dev-mode replica"]
  H -- "yes" --> J["engine.explain()"]
  J --> K{"policies list empty?"}
  K -- "yes" --> L["Nothing targets this action"]
  K -- "no" --> M{"targetMatch false?"}
  M -- "yes" --> N["Fix the policy target"]
  M -- "no" --> O{"conditionsMet false?"}
  O -- "yes" --> P["Read the conditions array"]
  O -- "no" --> Q["Another policy vetoed it"]
`}
/>
...
<Callout tone="warning" title="`defaultEffect: 'allow'` is a guard, not a mode">
...
<Callout tone="warning" title="First: is it wrong on every replica, or only some?">
...
<Callout tone="info" title="Prove the role reached the subject">
