---
title: Installation
description: Install @gentleduck/iam, pick the export paths you need with their peer dependencies, and run your first permission check
...
<Step>Declare your permission schema</Step>
...
<Step>Create an adapter and the engine</Step>
...
<Step>Ask the engine questions</Step>
...
<Step>Batch checks for a UI gate</Step>
...
<MermaidDiagram chart={`flowchart LR
    ROOT["@gentleduck/iam"] --> CORE["core: engine, builders, evaluators, types"]
    CORE --> COPT["core/validate, core/builder, core/explain, core/schema"]
    ROOT --- AD["adapters/*: memory, file, prisma, drizzle, redis, http"]
    ROOT --- SRV["server/*: express, nest, hono, next, generic"]
    ROOT --- CLI["client/*: react, vue, vanilla"]
    ROOT --- OPS["invalidators/redis, observability/metrics"]
    ROOT --- DT["dt and dt/v2: devtools panels"]
    AD --- DRZ["adapters/drizzle/{pg,mysql,sqlite}: schemas"]`} />
...
<PackageManagerTabs bun="bun add drizzle-orm" npm="npm i drizzle-orm" pnpm="pnpm add drizzle-orm" yarn="yarn add drizzle-orm" />

The v1 devtools panel needs only React:

<PackageManagerTabs bun="bun add -d react" npm="npm i -D react" pnpm="pnpm add -D react" yarn="yarn add -D react" />

The v2 panel needs the duck-ui trio as well:

<PackageManagerTabs bun="bun add -d react @gentleduck/libs @gentleduck/registry-ui lucide-react" npm="npm i -D react @gentleduck/libs @gentleduck/registry-ui lucide-react" pnpm="pnpm add -D react @gentleduck/libs @gentleduck/registry-ui lucide-react" yarn="yarn add -D react @gentleduck/libs @gentleduck/registry-ui lucide-react" />
...
<Callout tone="warning" title="The root barrel does not export adapters">
...
<Accordion type="multiple" collapsible className="w-full">
  <AccordionItem value="peer-dependencies-required">
    <AccordionTrigger>Do I need every peer dependency?</AccordionTrigger>
  ...
  <AccordionItem value="runtime-vs-monorepo-requirements">
    <AccordionTrigger>Why do the package requirements differ from the repository's tooling requirements?</AccordionTrigger>
    <AccordionContent className="text-muted-foreground">
  ...
  <AccordionItem value="need-create-iam">
    <AccordionTrigger>Do I have to use createIam() on day one?</AccordionTrigger>
    <AccordionContent className="text-muted-foreground">
  ...
  <AccordionItem value="autocomplete-not-there-yet">
    <AccordionTrigger>Why is autocomplete weak right after installing?</AccordionTrigger>
    <AccordionContent className="text-muted-foreground">
