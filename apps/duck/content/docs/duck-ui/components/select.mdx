...
Use `MotionSelect` and `MotionSelectContent` for smooth enter/exit animations powered by [motion](https://motion.dev). The dropdown enters and exits with a `scaleIn` preset and a bouncy spring transition. `MotionSelectTrigger` adds tap feedback (scale 0.97) to the trigger button.
...
Same props as `SelectContent`. Adds a `scaleIn` enter/exit animation with a `springBouncy` transition, mounted and unmounted via `useMotionRoot`/`useMotionMount` for exit-animation support. Must be used with `MotionSelect` root. Requires the `motion` package.
