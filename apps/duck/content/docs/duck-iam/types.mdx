---
title: Types and namespaces
description: Every exported namespace and member in @gentleduck/iam, with the exact flat and namespaced names and the import path each one comes from
section: Overview
---
...
<MermaidDiagram chart={`flowchart LR
    ROOT["@gentleduck/iam"] --> CORE["@gentleduck/iam/core"]
    CORE --> T1["AccessControl, IamRequest, IamPrimitives, IamClient, IamAdapter, DotPath"]
    CORE --> T2["IamConfig, IamEngineTypes, Evaluate, Explain"]
    CORE --> T3["IamValidate (type-only re-export)"]
    VAL["@gentleduck/iam/core/validate"] --> T3
    AD["adapters/*"] --> T4["IamMemory, IamFile, IamPrisma, IamDrizzle, Pg, Mysql, Sqlite, IamRedis, IamHttp"]
    SRV["server/*"] --> T5["IamExpress, IamHono, IamNest, IamNext, IamAdminAudit"]
    CLI["client/react"] --> T6["IamReactClient"]
    OBS["invalidators + observability"] --> T7["IamRedisInvalidator, IamMetrics"]`} />
...
<Callout tone="info" title="Why the React client re-declares map types">
...
<Callout tone="warning" title="The gated names are the public ones">
