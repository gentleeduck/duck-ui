...
Use `MotionTable` and `MotionTableRow` for staggered entrance animations powered by [motion](https://motion.dev). The table uses a `scaleIn` entrance with a bouncy spring transition, and each row staggers by 50ms via the `index` prop.
