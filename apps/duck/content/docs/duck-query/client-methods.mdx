...
description: Detailed reference for every Duck Query client method. Covers get, post, put, patch, del, request, byMethod, and the axios instance.
...
Sends a **PUT** request with a JSON body. Use it for full resource updates.
...
Sends a **PATCH** request with a JSON body, for partial updates rather than full replacement.
...
Pass the HTTP method as the first argument, which gives the strongest type checking.
